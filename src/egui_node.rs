use bevy::{
    ...
    render2::{
        ...
        render_resource::{
            ...
            BufferUsage, ColorTargetState, ColorWrite, Extent3d, FragmentState, FrontFace,
            IndexFormat, InputStepMode, LoadOp, MultisampleState, Operations,
...
use wgpu::{BufferBinding, BufferDescriptor, BufferSize};
...
impl Node for EguiNode {
    fn update(&mut self, world: &mut World) {
        ...
        for egui::ClippedMesh(rect, triangles) in &egui_paint_jobs {
            ...
            let texture_handle = match triangles.texture_id {
                egui::TextureId::Egui => egui_textures.main_textures[&self.window_id].clone(),
                // TOOD: handle textures correctly
                texture_id @ egui::TextureId::User(_) => {
                    egui_textures.user_textures[&texture_id].clone()
                }
            };
    ...
    fn run(
        ...
        // PERF: do this as a render app system?
...
impl EguiNode {
    fn create_texture_bind_group(
        ...
        let texture_handle_clone = texture_handle.clone_weak();
        self.image_bind_groups
            .entry(texture_handle)
            .or_insert_with(|| {
                let gpu_image = &image_render_assets[&texture_handle_clone];
