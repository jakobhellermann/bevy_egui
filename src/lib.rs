...
use bevy::{
    ...
    input::InputSystem,
    log,
    prelude::{App, Assets, Commands, Res, ResMut},
    render2::{render_graph::RenderGraph, texture::Image, RenderStage},
    utils::HashMap,
    window::WindowId,
};
...
// pub const EGUI_PIPELINE_HANDLE: HandleUntyped = todo!();
...
impl Plugin for EguiPlugin {
    fn build(&self, app: &mut App) {
        ...
        let render_app = &mut app.sub_app_mut(0);
...
impl Default for RenderGraphConfig {
    fn default() -> Self {
        RenderGraphConfig {
            ...
            run_before: bevy::render2::core_pipeline::node::MAIN_PASS_DEPENDENCIES,
...
pub fn setup_pipeline(
    ...
    /*msaa: &Msaa, */ config: RenderGraphConfig,
) {
    render_graph.add_node(config.egui_pass, EguiNode::new(config.window_id));

    render_graph
        .add_node_edge(
            bevy::render2::core_pipeline::node::MAIN_PASS_DRIVER,
            config.egui_pass,
        )
        .unwrap();
